...
| env[16] | object | `{"name":"DELIVERY_SKIP_TESTS","value":"true"}` | do not re-run tests under delivery |
| env[17] | object | `{"name":"DELIVERY_SAFE_MODE","value":"false"}` | if enabled no MR will be created |
| env[18] | object | `{"name":"USE_EMBEDDINGS_HTTP_API","value":"true"}` | use standalone embeddings (see aitestmate-embeddings chart) |
| env[19] | object | `{"name":"EMBEDDINGS_LLM_BASE_URL","value":"http://aitestmate-embeddings:7070"}` | embeddings endpoint (see aitestmate-embeddings chart) |
...
| image.repository | string | `"000000000000.dkr.ecr.us-east-1.amazonaws.com/epam-systems/add-ons/aitestmate/worker"` |  |
| image.tag | string | `""` |  |
